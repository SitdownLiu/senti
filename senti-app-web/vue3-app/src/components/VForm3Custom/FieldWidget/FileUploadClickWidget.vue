    ...
    >
      ...
      <template #file="{ file }">
        <div class="upload-file-list">
          ...
          <span class="file-action" :title="'查看'" @click="previewFile(file.name, file.url, file.uid)">
...
let selectFileText = "'选择文件'";

export default {
  name: 'file-upload-click-widget',
  ...
  methods: {
    ...
    previewFile(fileName, fileUrl, fileUid) {
      let foundIdx = -1;
      let foundFile = null;
      this.fileList.forEach((file, idx) => {
        if (file.name === fileName && (file.url === fileUrl || (!!fileUid && file.uid === fileUid))) {
      ...
      if (foundIdx >= 0) {
        if (!!this.field.options.onFilePreview) {
          let customFn = new Function('file', 'fileList', this.field.options.onFilePreview);
          customFn.call(this, foundFile, this.fileList);
        }
      }
    },
